...
The half-car model captures the vertical and pitch dynamics of a vehicle subject to suspension forces and road disturbances. The state variables are:
...
\begin{align}
	...
	f_4 &= \frac{1}{J}(f_{sf} d_f - f_{sr} d_r + u_2 + f_{wf} f + f_{wr} r)
\end{align}
...
\begin{align}
	...
	\end{bmatrix} =
	\begin{bmatrix}
		\sin x_3(f_2 + g) + \cos x_3 \frac{f_{wr} + f_{wf}}{m} \\
		\cos x_3(f_2 + g) - \sin x_3 \frac{f_{wr} + f_{wf}}{m} \\
		x_4 \\
		s_1 \\
		s_3
	\end{bmatrix} + \nu
...
We define the apparent pitch angle $\phi_a$ using accelerometer data:
...
\begin{tikzpicture}[scale=1.2, every node/.style={font=\small}]
	...
	% Sprung mass (body)
	\draw[thick, red] (-2,1.5) rectangle (3,2);
	\node at (-1,1.75) {$J, m$};
